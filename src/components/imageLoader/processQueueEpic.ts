...
const processQueueEpic: ImageLoaderEpic = (
  _action$,
  state$,
  { createStateObservable, dispatch },
) =>
  createStateObservable(
    ({ priorityQueue, processingQueue, standbyQueue }) => ({
      priorityQueue,
      processingQueue,
      standbyQueue,
    }),
    ...
    map(
      ({
        ...
        availableFilePaths: Array.from(
          new Set(
            Object.keys(priorityQueue).concat(
              Object.keys(standbyQueue),
            ),
          ),
        ),
        processingFilePaths: Object.keys(processingQueue),
      }),
    ),
    filter(
      ({ processingFilePaths }) =>
        processingFilePaths.length < processingQueueSize,
    ),
    map(({ availableFilePaths, processingFilePaths }) =>
      availableFilePaths.slice(
        0,
        processingQueueSize - processingFilePaths.length,
      ),
    ),
    mergeAll(),
    map((filePath) => ({
