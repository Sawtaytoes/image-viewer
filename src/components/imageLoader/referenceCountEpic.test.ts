...
import {
  ...
  type ImageLoaderEpic,
} from "./reduxObservable"
...
const createTestStore = () => {
  const dispatchedActions: ImageLoaderAction[] = []

  const captureEpic: ImageLoaderEpic = (action$) =>
    action$.pipe(
      tap((action) => {
  ...
  const typesFor = (actionType: string) =>
