...
const createPointerEvent = (
  type: string,
  { pointerType = "mouse" }: { pointerType?: string } = {},
) => {
...
describe("usePointerHover", () => {
  ...
  const renderPointerHover = () => {
    const callback =
      vi.fn<
        (notification: PointerHoverNotification) => void
      >()
  ...
  const lastIsHovering = (
    callback: ReturnType<typeof renderPointerHover>,
  ) => {
    const lastCall = callback.mock.calls.at(-1)

    if (!lastCall) {
      throw new Error(
        "usePointerHover never invoked its callback",
      )
    }
