...
const ImageViewControls = () => {
  const { imageFileName, leaveImageViewer } = useContext(
    ImageViewerContext,
  )
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      // Prevent taking screenshots in Windows with
      // `[META][SHIFT][S]` to shift the image.
      if (event.metaKey) {
        return
      }
    ...
    <div css={imageViewControlsStyles}>
      <div
        css={css`
					...
					${isAtBeginning && "visibility: hidden;"}
				`}
