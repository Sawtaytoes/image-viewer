...
// It is required now that `@charcuterie/ui` is in the graph: `ui` is a real
// React component library, and however React resolves on a given machine — a
// registry install, a `portal:` link to a local charcuterie checkout, or a
// second copy Vite's dep optimizer pre-bundles for `ui` — a SECOND React makes
// `@vitejs/plugin-react`'s Fast Refresh throw "can't detect preamble" (the
// preamble sets the refresh globals on one React; the component runs against the
// other), and the renderer mounts nothing → a blank window. Deduping forces a
// single React; it is a no-op when there is already only one. Mirror any change
// in `vitest.config.ts`.
